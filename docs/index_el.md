...
* [Λεπτομερείς πληροφορίες](#λεπτομερείς-πληροφορίες)
    ...
    * [Σχετικά](#σχετικά)
    * [Διαίρεση-Περικοπή](#διαίρεση-περικοπή)
    * [Μετονομασία](#μετονομασία)
    * [Περιοχή Ρίψης](#περιοχή-ρίψης)
    * [Δημιουργία βίντεο οπτικοποίησης](#δημιουργία-βίντεο-οπτικοποίησης)
...
Ο τομέας της Λίστας αναπαραγωγής είναι το κύριο μέρος του προγράμματος αναπαραγωγής.  
...
- **Προεπισκόπηση**  
  Από αυτό το κουμπί ανοίγει το παράθυρο της [Προεπισκόπησης](#προεπισκόπηση) για να ακούσουμε πιο κομμάτι θέλουμε για επόμενο.
- **Μίκτης**  
...
Αυτό το παράθυρο μας επιτρέπει να ακούσουμε το επόμενο κομμάτι, π.χ. πριν κάνουμε μια μίξη.  
Αυτή η αναπαραγωγή είναι ανεξάρτητη από την κανονική που παίζει τα κομμάτια, οπότε μπορούμε να τσεκάρουμε/σταματήσουμε οποιοδήποτε άλλο κομμάτι της λίστας, χωρίς να επηρεάζουμε τη φυσιολογική της ροή.  
Ο ήχος της προεπισκόπησης βγαίνει από την έξοδο Ακουστικών που έχουμε επιλέξει στις [Προτιμήσεις](#προτιμήσεις).  

Το παράθυρο περιέχει τα εξής:

- Η **Μπάρα θέσης** μας επιτρέπει να μετακινηθούμε μέσα σε ένα κομμάτι.  
  Όταν απλώς έχουμε το ποντίκι επάνω της, μας δείχνει τον χρόνο που έχει περάσει από την αρχή του κομματιού όπως επίσης και τον υπολειπόμενο χρόνο.
- Το κουμπί **Αναπαραγωγή/Παύση** του επιλεγμένου κομματιού.
- Το κουμπί **Ορισμός ως επόμενο** μετακινεί το επιλεγμένο κομμάτι αμέσως μετά από το κομμάτι που παίζει, ετοιμάζοντας το για σημάδεμα ή μιξάρισμα.
- Το **Σημάδεμα εκκίνησης** ορίζει τον [Χρόνο εκκίνησης](#dj-πάνελ), δηλαδή από ποιο σημείο θα αρχίσει η αναπαραγωγή του κομματιού όταν θα γίνει η μίξη.
- Η Οθόνη χρόνου εναλλάσσεται με κλικ μεταξύ του χρόνου από την αρχή του κομματιού είτε του εναπομείναντα χρόνου.
- Η μπάρα **Έντασης** αλλάζει την ένταση των ακουστικών. 
...
- **Διαίρεση/Περικοπή**  
    Ανοίγει ένα επιπλέον παράθυρο διαλόγου που σας επιτρέπει να [διαιρέσετε-περικόψετε](#διαίρεση-περικοπή) το κομμάτι σε  πολλαπλά κομμάτια ή σε ένα συγκεκριμένο μήκος. 
- **Αρχική θέση**  
...
- Ο **Τομέας Εξόδου** μας επιτρέπει να επιλέξουμε ανεξάρτητα την κύρια έξοδο ήχου για τα ηχεία και την έξοδο για τα ακουστικά που χρησιμοποιεί η [Προεπισκόπηση](#προεπισκόπηση).
...
- **Εμφάνιση […] πρόσφατων κομματιών**  
  Ο αριθμός των κομματιών που εμφανίζεται στο μενού των Πρόσφατων κομματιών.
- **Ταχύτητα**  
  Η ταχύτητα αναπαραγωγής των κομματιών. Η κανονική είναι x1.00.
  Εάν η ταχύτητα αλλάξει, ο επεξεργασμένος ήχος μπορεί να εξαχθεί από το δεξί-κλικ μενού του κομματιού (Εξαγωγή επεξεργασμένου ήχου).
- **Τονικότητα**  
  Αλλαγή της τονικότητας των κομματιών σε ημιτόνια. Χωρίς καμία αλλαγή είναι το 0.00.
  Εάν η τονικότητα αλλάξει, ο επεξεργασμένος ήχος μπορεί να εξαχθεί από το δεξί-κλικ μενού του κομματιού (Εξαγωγή επεξεργασμένου ήχου).
- **Έξοδος σε εικονίδιο**  
...
Όταν ανοίγουμε για πρώτη φορά αυτό το παράθυρο, περιέχει μόνο ένα μέρος με τον χρόνο της αρχής του (που είναι και ο χρόνος αρχής του κομματιού), και ένα δεύτερο χρόνο που είναι ο χρόνος για το **Τέλος του κομματιού**.  
Οι αρχικές τιμές αυτών των χρόνων είναι 00:00:00 για την αρχή και η διάρκεια του κομματιού για το τέλος.  
Αλλάζοντας αυτούς τους χρόνους κόβουμε από την αρχή και το τέλος του κομματιού αντίστοιχα.  
...
Κάνοντας διπλό-κλικ στη στήλη **Τίτλος** οποιουδήποτε μέρους, μπορούμε να αλλάξουμε τον τίτλο του, και κατά συνέπεια να αλλάξουμε τον τίτλο/το όνομα αρχείου του κομματιού που προκύπτει.  
Το πεδίο **Αρχή** όλων των μερών είναι επεξεργάσιμο και μπορούμε να κάνουμε προεπισκόπηση των αλλαγών μας χρησιμοποιώντας το κουμπί **Προεπισκόπηση**.  
...
Ο Τίτλος θα είναι ο τίτλος του μέρους, και αν τσεκάρουμε το τσεκ με την **Αρίθμηση τμημάτων**, ο αριθμός του μέρους θα είναι ο αριθμός του κομματιού στα μεταδεδομένα τους.  
...
### Δημιουργία βίντεο οπτικοποίησης

![VideoSetup.png](images/VizVideoSetup.png)


Το πρώτο πράγμα που μπορούμε να επιλέξουμε είναι η ανάλυση του βίντεο.  
Μπορούμε επίσης να ελέγξουμε τι θα δείχνει το βίντεο, επιλέγοντας ποιά μεταδεδομένα θα συμπεριλάβουμε και ποια από τα στοιχεία οπτικοποίησης θα ενεργοποιήσουμε.  
Μπορούμε να διαλέξουμε και τη γραμματοσειρά που θα χρησιμοποιηθεί.  

Με κάθε αλλαγή που κάνουμε, το δεξί μέρος του παραθύρου ανανεώνεται ώστε να μας δείξει πως θα φαίνεται ένα τυχαίο καρέ του βίντεο.  
Ας λάβουμε υπόψιν μας όμως ότι αυτή είναι μια στατική προεπισκόπηση. Το βίντεο θα δείχνει τα στοιχεία οπτικοποίησης συγχρονισμένα με τον ήχο του κομματιού.  

Πατώντας το κουμπί **Δημιουργία** θα μας ζητηθεί η διαδρομή στην οποία θα αποθηκευτεί το βίντεο, και μετά θα ξεκινήσει η κατασκευή του.
...
|:----------------------------------------------:|:------------------------------------------------:|:------------------------------------------------------------------------------------------------:|
|        _Επιλεγμένο ένα κομμάτι αρχείου_        |       _Επιλεγμένα πολλά κομμάτια αρχείων_        | _Ένα κομμάτι αρχείου <br/>επιλεγμένο (παίζει ή έχει <br/>αποθηκευμένο Σημείο <br/>αναπαραγωγής)_ |
...
- **[Λιστα](#μενού-λίστας-αναπαραγωγής)** (υπό-μενού)
...
- Η επιλογή **[Δημιουργία βίντεο οπτικοποίησης](#δημιουργία-βίντεο-οπτικοποίησης)**, ανοίγει ένα παράθυρο που μας επιτρέπει να δημιουργήσουμε ένα βίντεο χρησιμοποιώντας οποιονδήποτε συνδυασμό οπτικοποιητών και των μεταδεδομένων του κομματιού συγχρονισμένα με τον ήχο του. Αυτή η επιλογή εμφανίζεται όταν έχουμε επιλέξει μόνο ένα κομμάτι από τον σκληρό μας.  
...
- Το μενού **Αναλυτής φάσματος**, μας επιτρέπει να ρυθμίσουμε την εμφάνιση του. Έχει 3 υπό-μενού.
  - Το υπό-μενού **Αριθμός στηλών**, μας επιτρέπει να επιλέξουμε τον αριθμό των γραμμών που θα χρησιμοποιηθούν από τον αναλυτή φάσματος. Η προεπιλογή είναι 32, αλλά μπορούμε να επιλέξουμε μια τιμή μεταξύ 24, 32, 40, 48 και 56.
  - Το υπό-μενού **Διατήρηση κορυφών** ενεργοποιεί (ή όχι) την ύπαρξη κορυφών του αναλυτή.
  - Το υπό-μενού **Βελτιστοποίηση ύψους** ενεργοποιεί (ή όχι) τη μεγιστοποίηση του αναλυτή ανεξάρτητα από την ένταση της κυματομορφής στο συγκεκριμένο σημείο.
- Το μενού **Παλμογράφος**, με το υπό-μενού **Μεγέθυνση**, όπου μπορούμε να επιλέξουμε διαφορετική μεγέθυνση του παραθύρου της δειγματοληψίας του:
  - Χαμηλή
  - Κανονική
  - Μεγάλη
...
| Ταχύτητα αναπαραγωγής κάτω ανά 0.1     | [ ή / (Shift για cents, Alt για 1.0) |
| Ταχύτητα αναπαραγωγής πάνω ανά 0.1     | ] ή * (Shift για cents, Alt για 1.0) |
| Τονικότητα αναπαραγωγής ημιτόνιο κάτω  | - ή _ (Shift για cents, Alt για 0.0) |
| Τονικότητα αναπαραγωγής ημιτόνιο πάνω  | = ή + (Shift για cents, Alt για 0.0) |
